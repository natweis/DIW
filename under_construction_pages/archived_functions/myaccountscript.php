...
class Myaccount
{
    ...
    {  
            $db=new Dbconnect;
            $db->connect();
            $sql="SELECT * From customers, customerlogon WHERE customers.CustomerID=customerlogon.CustomerID AND customers.Email='$username'";
            $stmt = $db->prepareStatement($sql);
            $stmt->execute();

            $all_account=$stmt->fetchall();
            ...
            foreach($all_account as $account)
	        {
				...
				$datelastmodified=$account["DateLastModified"];
				
				echo "<div id='acc'><tr class='text-center'>";
				echo "<div><tr><td class='text-center'>$customerid</td>";
                echo "<td> $firstname</td>"; 
                echo "<td> $lastname</td>"; 
                echo "<td> $email</td>"; 
                echo "<td> $datejoined</td>"; 
                echo "<td> $datelastmodified</td>";  
				echo "</tr></div>";
				echo "<div><tr><th class='text-center' colspan='7'>";
                    ...
                    echo "<input  type='text' name='postal' value='$postal' required>";
                    echo "<p id='acctext'>Stadt*</p>";
                    echo "<input  type='text' name='city' value='$city' required>";
                    ...
                    echo "<select name='country'  required>
                        ...
                        echo "<input  type='tel' name='phone' value='$phone' required>";
                    ...
                    echo "</details>";
                        ...
                        echo "<form method='post'>";
                        echo "<input type='hidden' name='customerid' value=$customerid>";
                        echo "<p id='acctext'>Altes Passwort*</p>";
                        echo "<input type='password' name='pwold' placeholder='altes Passwort'  required><br/>";
    ...
    public function userupdate()
    {
        ...
        {   
            ...
            $stmt->execute();
            return "Nutzerdaten erfolgreich geändert";
            $db->close();
        }
    ...
    public function changepassword()
    {
        if(isset($_POST["submitchangepassword"]))
        {
            $db=new Dbconnect;
            $db->connect();
            $sql="SELECT * FROM customerlogon WHERE CustomerID=:customerid";
            $stmt = $db->prepareStatement($sql);
            $stmt->bindValue(":customerid",$_POST["customerid"]);
            $stmt->execute();
            $row=$stmt->fetch();
            if(Password_verify($_POST["pwold"], $row["Pass"]))
            {
